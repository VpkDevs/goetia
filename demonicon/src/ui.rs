...
// Slot loops index several parallel arrays (skills/sigils/keys) and drive
// layout math off the same index; `needless_range_loop` doesn't apply.
...
pub fn draw_hud_run(eng: &mut Engine, gs: &mut Gs, rs: &RunState, ui: &mut UiBatch) {
    ...
    ui.text_shadowed(
        Vec2::new(bx + 6.0, by + 6.0),
        2.0,
        Vec4::new(0.05, 0.05, 0.08, 1.0),
        &format!("{:.0}/{:.0}", hp.max(0.0), max),
    );
        ...
        match &gs.loadout.skills[i] {
            Some(id) => {
                ...
                let name_short: String = def
                    ...
                    .collect();
                let color = if ready {
                    c4(dominant_type(&def.dmg_vec()).color())
                } else {
                    DIM
                };
                ...
                if !ready {
                    ...
                    ui.rect(
                        Vec2::new(x, sy + 56.0 - 52.0 * f),
                        Vec2::new(slot_w - 6.0, 52.0 * f),
                        Vec4::new(0.0, 0.0, 0.0, 0.55),
                    );
                }
                // Sigil pips.
                ...
                for s in 0..socketed {
                    ui.rect(
                        Vec2::new(x + 4.0 + s as f32 * 10.0, sy + 44.0),
                        Vec2::new(7.0, 7.0),
                        c4(palette::GOLD),
                    );
                }
    ...
    let rx = vp.x - 250.0;
    ui.text_shadowed(
        Vec2::new(rx, vp.y - 88.0),
        1.6,
        c4(rs.demon.signature()),
        rs.demon.name(),
    );
    ui.text_shadowed(
        Vec2::new(rx, vp.y - 68.0),
        1.5,
        WHITE,
        &format!("TIER {}", gs.tier),
    );
    ui.text_shadowed(
        Vec2::new(rx, vp.y - 50.0),
        1.5,
        c4(palette::GOLD),
        &format!("DUST {}", gs.dust),
    );
    ui.text_shadowed(
        Vec2::new(rx, vp.y - 32.0),
        1.5,
        DIM,
        &format!("KILLS {}", gs.pc.kills_this_run),
    );
    ...
    for md in &rs.mods {
        ui.text_shadowed(
            Vec2::new(vp.x - 8.0 - UiBatch::text_width(1.2, &md.text), y),
            1.2,
            DIM,
            &md.text,
        );
    ...
    if rs.demon == Demon::Buer {
        let (t, c) = if gs.blight_phase {
            ("BLIGHT", palette::ICHOR)
        } else {
            ("BLOOM", palette::GOLD)
        };
        ui.text_shadowed(
            Vec2::new(vp.x - 8.0 - UiBatch::text_width(2.0, t), y + 6.0),
            2.0,
            c4(c),
            t,
        );
    ...
    if eng.world.is_alive(rs.boss) {
        if let Some(h) = eng.world.get::<Health>(rs.boss).copied() {
            ...
            if engaged {
                ...
                let label = if immune {
                    format!("{name} - IMMUNE UNTIL BLIGHT")
                } else {
                    name
                };
                ui.text_shadowed(
    ...
    if let Some(h) = hint {
        ui.text_shadowed(
            Vec2::new(
                vp.x * 0.5 - UiBatch::text_width(1.8, &h) * 0.5,
                vp.y - 140.0,
            ),
            1.8,
            c4(palette::GOLD),
    ...
    if rs.cleared {
        ...
        ui.text_shadowed(
            Vec2::new(vp.x * 0.5 - UiBatch::text_width(1.6, t) * 0.5, 52.0),
            1.6,
            c4(palette::GOLD),
            t,
        );
    ...
    ui.text_shadowed(
        Vec2::new(8.0, vp.y - 20.0),
        1.2,
        DIM,
        "TAB: SPOILS  WASD: MOVE  MOUSE: AIM  SPACE: DODGE  G: INTERACT",
    );
}

pub fn draw_hud_court(
    eng: &mut Engine,
    gs: &mut Gs,
    ui: &mut UiBatch,
    sel: usize,
    tier: u32,
    progress: &crate::save::Progress,
) {
    let vp = eng.viewport;
    let title = "THE GOETIC COURT";
    ui.text_shadowed(
        Vec2::new(vp.x * 0.5 - UiBatch::text_width(3.0, title) * 0.5, 18.0),
        3.0,
        WHITE,
        title,
    );
    ...
    ui.text_shadowed(
        Vec2::new(vp.x * 0.5 - UiBatch::text_width(1.4, sub) * 0.5, 46.0),
        1.4,
        DIM,
        sub,
    );
    ...
    ui.text_shadowed(
        Vec2::new(
            vp.x * 0.5 - UiBatch::text_width(1.7, &line) * 0.5,
            vp.y - 60.0,
        ),
        1.7,
        WHITE,
...
fn inv_item(gs: &Gs, in_run: bool, i: usize) -> Option<&ItemInstance> {
    if in_run {
        gs.run_inv.get(i)
    } else {
        gs.bank.get(i)
...
pub fn menu_input(eng: &mut Engine, gs: &mut Gs, ms: &mut MenuState, in_run: bool) -> bool {
    if eng.input.key_pressed(KeyCode::Tab) {
        ms.open = !ms.open;
        eng.audio
            .play(&gs.sounds.ui, "sfx", 0.4, if ms.open { 1.2 } else { 0.8 });
    }
    if !ms.open {
        return false;
    }
            ...
            if enter {
                if let Some(item) = inv_remove(gs, in_run, ms.cursor) {
                    ...
                    let slot_idx = match item.slot {
                        ...
                        Slot::Relic => 2,
                        Slot::Ring => {
                            if gs.loadout.equipment[3].is_none() {
                                3
                            } else if gs.loadout.equipment[4].is_none() {
                                4
                            } else {
                                3
                            }
            ...
            if in_run {
                ...
                for (k, sock) in [
                    ...
                    if eng.input.key_pressed(k) {
                        let ids: Vec<String> =
                            gs.db.sigils().iter().map(|s| s.id.clone()).collect();
...
pub fn draw_menu(eng: &mut Engine, gs: &mut Gs, ms: &MenuState, ui: &mut UiBatch, in_run: bool) {
    ...
    // Pane tabs.
    let tabs = [
        ("SPOILS", Pane::Inventory),
        ("VESTMENTS", Pane::Equipment),
        ("RITES", Pane::Skills),
        ("PACTS", Pane::Contracts),
    ];
    let mut tx = x0 + 16.0;
    for (name, p) in tabs {
        let sel = ms.pane == p;
        ui.text(
            Vec2::new(tx, y0 + 12.0),
            2.0,
            if sel { c4(palette::GOLD) } else { DIM },
            name,
        );
    ...
    match ms.pane {
        Pane::Inventory => {
            ...
            ui.text(
                Vec2::new(list_x, list_y - 24.0),
                1.3,
                DIM,
                if in_run {
                    "RUN SPOILS (UNBANKED - DEATH EATS THESE)"
                } else {
                    "BANKED SPOILS"
                },
            );
            if n == 0 {
                ui.text(
                    Vec2::new(list_x, list_y),
                    1.5,
                    DIM,
                    "NOTHING. GO TAKE SOMETHING.",
                );
            }
            ...
            for (row, i) in (first..n.min(first + 22)).enumerate() {
                ...
                let sel = i == ms.cursor;
                let c = if sel {
                    c4(item.rarity.color())
                } else {
                    c4(item.rarity.color() * 0.6)
                };
            ...
            if let Some(item) = inv_item(gs, in_run, ms.cursor) {
                ...
                for (li, (text, col)) in lines.iter().enumerate() {
                    ui.text(
                        Vec2::new(card_x, list_y + li as f32 * line_h),
                        1.5,
                        c4(*col),
                        text,
                    );
        ...
        Pane::Equipment => {
            for i in 0..EQUIP_SLOTS {
                ...
                ui.text(
                    Vec2::new(list_x + 130.0, list_y + i as f32 * line_h * 1.4),
                    1.6,
                    color,
                    &name,
                );
            ...
            for (i, (name, key, pct)) in stats.iter().enumerate() {
                ...
                ui.text(
                    Vec2::new(list_x + (i % 4) as f32 * 160.0, sy + (i / 4) as f32 * 22.0),
                    1.4,
                    WHITE,
                    &txt,
                );
        ...
        Pane::Skills => {
            if in_run {
                ui.text(
                    ...
                    1.6,
                    DIM,
                    "RITES ARE REWRITTEN AT COURT ONLY.",
                );
            }
            let keys = ["LMB", "RMB", "Q", "E", "R", "F"];
            for i in 0..6 {
                ...
                ui.text(
                    ...
                    1.6,
                    if sel { WHITE } else { DIM },
                    &format!("{} {:<4} {}", if sel { ">" } else { " " }, keys[i], name),
                );
                ...
                for s in 0..3 {
                    ...
                    let c = if gs.loadout.sigils[i][s].is_some() {
                        c4(palette::GOLD)
                    } else {
                        DIM
                    };
            ...
            if !in_run {
                ui.text(
                    ...
                    1.4,
                    c4(palette::GOLD),
                    "ENTER: CYCLE RITE   1/2/3: CYCLE SIGIL SOCKETS   RESPEC IS FREE. ALWAYS.",
                );
            }
        }
        Pane::Contracts => {
            if in_run {
                ui.text(
                    ...
                    1.6,
                    DIM,
                    "PACTS ARE SIGNED AT COURT ONLY.",
                );
            }
            for i in 0..3 {
                let sel = i == ms.cursor;
                ...
                match &gs.loadout.contracts[i] {
                    Some(id) => {
                        ...
                        ui.text(
                            Vec2::new(list_x, y),
                            1.6,
                            if sel { c4(c.demon.signature()) } else { DIM },
                            &format!("{} {}", if sel { ">" } else { " " }, c.name),
                        );
                        ui.text(Vec2::new(list_x + 30.0, y + 18.0), 1.2, WHITE, &c.text);
                        ui.text(
                            Vec2::new(list_x + 30.0, y + 34.0),
                            1.1,
                            DIM,
                            &format!("- {}", c.demon.name()),
                        );
            ...
            if !in_run {
                ui.text(
                    ...
                    1.4,
                    c4(palette::GOLD),
                    "ENTER: CYCLE PACT. THREE AT MOST. READ THE FINE PRINT OR DON'T.",
                );
            }
        }
    }
...
pub fn draw_death(eng: &mut Engine, ui: &mut UiBatch, timer: u16, lost: usize) {
    ...
    ui.text_shadowed(
        Vec2::new(vp.x * 0.5 - UiBatch::text_width(3.4, t) * 0.5, vp.y * 0.4),
        3.4,
        c4(palette::BLOOD),
        t,
    );
    ...
    ui.text_shadowed(
        Vec2::new(
            vp.x * 0.5 - UiBatch::text_width(1.6, &l) * 0.5,
            vp.y * 0.4 + 44.0,
        ),
        1.6,
        WHITE,
    ...
    ui.text_shadowed(
        Vec2::new(
            vp.x * 0.5 - UiBatch::text_width(1.6, &s) * 0.5,
            vp.y * 0.4 + 70.0,
        ),
        1.6,
        DIM,
